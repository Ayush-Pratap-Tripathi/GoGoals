...
const ChartBlock = ({ title, data, onPrevious, onNext, isPremium = true }) => {
  ...
  const { handlePremiumAttempt, elementRef } = usePremiumFeature("Upgrade to premium to use this feature");
  ...
  const handleWheel = (e) => {
    ...
    if (Math.abs(e.deltaX) > Math.abs(e.deltaY)) {
      ...
      if (e.deltaX > 25) {
        ...
        }, 500); // Prevents infinite fast-forward tracking
      } else if (e.deltaX < -25) {
        swipeLockRef.current = true;
        handlePrev();
        setTimeout(() => {
  ...
  return (
    ...
    >
      ...
      <div
        ref={elementRef}
        className={`h-64 md:h-[400px] w-full touch-pan-x cursor-grab active:cursor-grabbing relative`}
        onWheel={handleWheel}
      >
